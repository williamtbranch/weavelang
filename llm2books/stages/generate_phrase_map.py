...
class GeneratePhraseMap(LLMStage):
    # ... (keep __init__, get_system_prompt, prepare_llm_items) ...
    ...
    def run(self) -> bool:
        ...
        if self.output_path.exists():
            ...
            except Exception: logger.warning(f"      -> Could not parse existing output file. Re-running.")
        ...
        for i in range(0, len(items_for_this_run), batch_size):
            ...
            raw_response_map = {item['id']: item['llm_response'] for item in llm_results_list}
            parsed_clean_map = llm_utils._parse_multiline_llm_response(raw_response_map[batch_items[0]['id']])
            
            # Handle anonymous block by assigning it to the first item of the batch
            if "ANONYMOUS_BLOCK" in parsed_clean_map:
                first_item_id = batch_items[0]['id']
                logger.warning(f"      -> LLM did not prefix its response. Assigning anonymous block to first item ID: {first_item_id}")
                final_map = {first_item_id: parsed_clean_map["ANONYMOUS_BLOCK"]}
                # Add any other correctly parsed items from the same response
                for key, val in parsed_clean_map.items():
                    if key != "ANONYMOUS_BLOCK": final_map[key] = val
            else:
                final_map = parsed_clean_map

            for item in batch_items:
                s_id = item['id']
                clean_mapping_str = final_map.get(s_id, "")
                
                if not _validate_llm_map(s_id, item['text'], clean_mapping_str):
                    logger.error(f"Validation failed for S_ID {s_id}. Halting pipeline. Check LLM log for '{self.stage_name}'.")
                    return False
            ...
            if not self._save_output_data(input_data, "PARTIAL"):
                logger.error("CRITICAL: Failed to save progress to disk after a successful batch. Halting.")
                return False
    ...
    def process_llm_results_for_block(self, block: Dict, llm_results: Dict[str, str]) -> Dict:
        ...
        if s_id in llm_results:
            # llm_results now contains the CLEAN mapping string
...
class GeneratePhraseMap(LLMStage):
    """
    New Stage 4: Generates a phrase-based mapping from the base language
