...
pub fn render(ui: &mut egui::Ui, state: &mut AppState) {
    ui.horizontal(|ui| {
        ...
        selectable_view_tab(ui, state, "Source", TierView::Base);
        ...
        if ui
            .selectable_label(state.left_view == TierView::Simulation, "Simulation")
            .clicked()
        {
            state.pending_terminal_command = Some("set left_view simulation".to_string());
        }
        ...
        if ui
            .button("🔌 Test Bridge")
            .on_hover_text("Check Python Connection")
            .clicked()
        {
        ...
        if ui
            .button("🧠 Test LLM")
            .on_hover_text("Ping Anthropic API (Costs tokens!)")
            .clicked()
        {
        ...
        ui.separator();
        if state.llm_results_receiver.is_some() {
            ui.spinner();
            ui.colored_label(
                egui::Color32::from_rgb(200, 160, 0),
                format!("LLM job running — {}/{} done", state.llm_job_done, state.llm_job_total),
            );
        } else {
...
fn selectable_view_tab(ui: &mut egui::Ui, state: &mut AppState, label: &str, view: TierView) {
    if ui
        .selectable_label(state.left_view == view, label)
        .clicked()
    {
        ...
        state.pending_terminal_command = Some(format!("set left_view {}", view_str));
    }
}
