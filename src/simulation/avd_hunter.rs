...
pub fn run_hunt(
    ...
    for user_level in 1..=max_user_levels {
        ...
        println!("  -> Performing binary search for {:.2}% density...", NEW_LEMMA_DENSITY_TARGET * 100.0);
        ...
        let (lemmas, eng_words) = generate_and_measure_at_locked_level(
            &numerical_chapter,
            &dictionary,
            v_n_plus_1,
        );
        ...
        let new_words_tally = (final_metrics.calculate_new_lemma_density(previous_v_level) * total_words as f64).round() as u64;

        println!(
            "  -> Discovered User Level {}: V-Level = {}, AVD Score = {:.2}",
            user_level, v_n_plus_1, avd_score
        );
        println!(
        ...
        for lemma_str in &lemmas {
            if let Some(rank) = frequency_manager::get_rank_for_lemma(lemma_str) {
                if rank > previous_v_level {
                    new_lemmas_found.insert(format!("'{}' (Rank: {})", lemma_str, rank));
                }
            }
        }
        ...
        if !new_lemmas_found.is_empty() {
            ...
            if sorted_new_lemmas.len() % 4 != 0 { println!(); }
        } else {
    ...
    for (ul, vl, avd) in master_scale {
        writer.write_record(&[ul.to_string(), vl.to_string(), format!("{:.4}", avd)])?;
    }
    writer.flush()?;
