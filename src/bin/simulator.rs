...
mod production {
    ...
    impl ProductionSim {
        ...
        fn get_rate_from_curve(&self, current_percent: f64) -> u64 {
            if self.rate_curve.is_empty() { return 900; }
            for window in self.rate_curve.windows(2) {
                let (p1, r1) = window[0]; let (p2, r2) = window[1];
        ...
        pub fn run(&mut self) -> Vec<Checkpoint> {
            ...
            loop {
                let current_intro_percent = (self.next_lemma_to_introduce_idx as f64 / VOCAB_SIZE as f64) * 100.0;
                let target_rate = self.get_rate_from_curve(current_intro_percent);
                while self.words_processed_since_last_intro >= target_rate && self.next_lemma_to_introduce_idx < VOCAB_SIZE as u32 {
                    self.active_lemmas.insert(self.next_lemma_to_introduce_idx, 0);
                ...
                for _ in 0..WORD_PROCESSING_BATCH_SIZE {
                    ...
                    let seen_word_id = if self.known_lemmas.contains(&original_word_id) || self.active_lemmas.contains_key(&original_word_id) {
                ...
                let graduated: Vec<u32> = self.active_lemmas.iter().filter(|(_, &c)| c >= EXPOSURE_THRESHOLD).map(|(&id, _)| id).collect();
                ...
                let ka_vocab_percent = (self.next_lemma_to_introduce_idx as f64 / VOCAB_SIZE as f64) * 100.0;
                if ka_vocab_percent >= next_checkpoint_percent {
                    let hours = self.words_processed as f64 / WORDS_PER_HOUR as f64;
                    let rate = if hours > 0.0 { self.known_lemmas.len() as f64 / hours } else { 0.0 };
                    let known_count = self.known_lemmas.len();
                    let active_count = self.active_lemmas.len();
                    let total_vocab = known_count + active_count;
                    let comprehension = if total_vocab > 0 { known_count as f64 / total_vocab as f64 } else { 1.0 };
                    checkpoints.push(Checkpoint { ka_vocab_percent, known_words: known_count, active_words: active_count, hours_elapsed: hours, learning_rate: rate, measured_comprehension: comprehension * 100.0 });
                    next_checkpoint_percent += 5.0;
                }
                if self.next_lemma_to_introduce_idx >= VOCAB_SIZE as u32 { break; }
            }
            checkpoints
        }
    }
...
fn main() {
    ...
    println!("Simulating learner journey with a fixed introduction curve and fair substitution model.");
    
    let rate_curve = vec![
        (0.0, 3000), (5.0, 550), (10.0, 590), (15.0, 620), (20.0, 640), (25.0, 660), 
        (30.0, 670), (35.0, 680), (40.0, 690), (45.0, 700), (50.0, 700), (55.0, 715), 
        (60.0, 720), (65.0, 720), (70.0, 730), (75.0, 730), (80.0, 740), (85.0, 740), 
        (90.0, 745), (95.0, 750), (100.0, 755)
    ];
    ...
    println!("{:<8} | {:<8} | {:<8} | {:<8} | {:<9} | {:<12} | {:<12}", "K+A (%)", "Known", "Active", "Total", "Hours", "Learn Rate", "Comp. (%)");
    println!("{}", "-".repeat(80));
    if let Some(first_run) = prod_checkpoints.first() {
        for i in 0..first_run.len() {
            let percent = first_run[i].ka_vocab_percent;
            let num_runs = prod_checkpoints.iter().filter(|r| r.get(i).is_some()).count();
            if num_runs > 0 {
                let avg_known = prod_checkpoints.iter().filter_map(|r| r.get(i)).map(|c| c.known_words).sum::<usize>() / num_runs;
                let avg_active = prod_checkpoints.iter().filter_map(|r| r.get(i)).map(|c| c.active_words).sum::<usize>() / num_runs;
                let avg_hours = prod_checkpoints.iter().filter_map(|r| r.get(i)).map(|c| c.hours_elapsed).sum::<f64>() / num_runs as f64;
                let avg_rate = prod_checkpoints.iter().filter_map(|r| r.get(i)).map(|c| c.learning_rate).sum::<f64>() / num_runs as f64;
                let avg_comp = prod_checkpoints.iter().filter_map(|r| r.get(i)).map(|c| c.measured_comprehension).sum::<f64>() / num_runs as f64;
                println!("{:<7.1}% | {:<8} | {:<8} | {:<8} | {:<8.0} | {:<11.2} w/h | {:<11.2} %", percent, avg_known, avg_active, avg_known + avg_active, avg_hours, avg_rate, avg_comp);
            }
        }
    }
}