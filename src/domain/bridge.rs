...
pub fn json_to_domain_sentence(json_block: &JsonSentenceBlock) -> Result<Sentence, Box<dyn Error>> {
            ...
            for json_token in &json_segment.tokenized_text {
                let token = match json_token.token_type {
                    JsonTokenType::Background => {
                        Token::Background(json_token.value.clone())
                    },
                    JsonTokenType::Word => {
    ...
    if sentence.get_tier("basic_base").is_some() {
        let mut forward_mapping = TierMapping::new("basic_base".to_string(), "basic_target".to_string());
        for (_, entries) in &json_block.mappings.basic_diglot {
            for entry in entries {
                ...
                let mut map_entry = MappingEntry::new(source_id, target_text.clone(), target_lemmas.clone());
        ...
        if !forward_mapping.entries.is_empty() { sentence.add_mapping(forward_mapping); }
    }

    if sentence.get_tier("basic_target").is_some() {
        let mut inverse_mapping = TierMapping::new("basic_target".to_string(), "basic_base".to_string());
        for (_, entries) in &json_block.mappings.basic_inverse_diglot {
            for entry in entries {
                let word_idx = entry.0;
                let target_lemmas = &entry.1;
                let target_text = &entry.2;
                let source_id = WordId(word_idx as u64);
                let map_entry = MappingEntry::new(source_id, target_text.clone(), target_lemmas.clone());
                inverse_mapping.add_entry(map_entry);
            }
        }
        if !inverse_mapping.entries.is_empty() { sentence.add_mapping(inverse_mapping); }
    }
